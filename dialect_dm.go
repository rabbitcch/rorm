...
func (s *dm) DataTypeOf(field *StructField) string {
	...
	if sqlType == "" {
		panic(fmt.Sprintf("invalid sql type %s (%s) for dm", dataValue.Type().Name(), dataValue.Kind().String()))
	}
...
func (s dm) LimitAndOffsetSQL(limit, offset interface{}) (sql string) {
	if limit != nil {
...
func (s dm) BuildForeignKeyName(tableName, field, dest string) string {
